...
SYSTEM_PROMPT = (
    ...
    "the normalised name, its monthly charge amounts (oldest first), the charge cadence, and "
    "sample card descriptors and memos. Assign exactly one cost type and one category.\n\n"
    "Cost types:\n"
...
def _user_message(facts: list[VendorFacts]) -> str:
    rows = [
        {
            "vendor": f.vendor,
            "monthly_amounts": [round(a, 2) for a in f.monthly_amounts],
            "cadence": f.cadence,
